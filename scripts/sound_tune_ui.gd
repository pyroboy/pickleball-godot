class_name SoundTuneUI extends Node
...
func setup(parent_canvas: CanvasLayer) -> void:
	_create_sound_tune_panel(parent_canvas)
...
func _refresh_sound_tune_panel() -> void:
	if _main.ball == null:
		return
	var tunings: Dictionary = _main.ball.audio_synth.get_sound_tunings()
	for i in range(sound_tune_settings.size()):
		...
		if i < sound_tune_rows.size():
			...
			if id == "paddle_pitch" and _main.ball.has_method("get_paddle_pitch_note") and _main.ball.has_method("get_paddle_pitch_frequency"):
				label_text += "  [" + _main.ball.audio_synth.get_paddle_pitch_note() + " / " + str(int(round(_main.ball.audio_synth.get_paddle_pitch_frequency()))) + " Hz]"
...
func _adjust_sound_tuning(delta: float) -> void:
	...
	# TODO: will go through audio_synth — _main.ball.audio_synth.set_sound_tuning()
	_main.ball.audio_synth.set_sound_tuning(id, next_value)
	_refresh_sound_tune_panel()
...
func _trigger_test_sound_key(keycode: Key, key_name: String, sound_type: String, sound_index: int) -> void:
	...
	if pressed and not was_pressed:
		if sound_type == "paddle":
			# TODO: will go through audio_synth — _main.ball.audio_synth.play_test_paddle_sound()
			_main.ball.audio_synth.play_test_paddle_sound(sound_index)
		else:
			# TODO: will go through audio_synth — _main.ball.audio_synth.play_test_court_sound()
			_main.ball.audio_synth.play_test_court_sound(sound_index)
